...
Try {
	...
	[string]$appName = 'Call Center Supervisor'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "Setup.exe" -Parameters "/S /v`"/qn REBOOT=reallysuppress`"" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path='{BE6CBCA1-4962-4DF7-8E1D-D5A0FCBEAE27}' -WindowStyle "Hidden" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
